### From: https://github.com/thuml/Time-Series-Library/blob/main/models/TiDE.py
...
class Model(nn.Module):  
    ...
    def __init__(self, configs, bias=True, feature_encode_dim=2): 
        ...
        self.decode_dim = configs.enc_in + 1#configs.c_out
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, batch_y_mark):
        ...
        x_enc = x_enc - means
        stdev = torch.sqrt(torch.var(x_enc.clone(), dim=1, keepdim=True, unbiased=False) + 1e-5)
        x_enc = x_enc / stdev
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            x_mark_dec=torch.concat([x_mark_enc, x_mark_dec[:, -self.pred_len:, :]],dim=1)
            dec_out = torch.stack([self.forecast(x_enc[:, :, feature], x_mark_enc, x_dec, x_mark_dec) for feature in range(x_enc.shape[-1])],dim=-1)
            return dec_out # [B, L, D]
        if self.task_name == 'imputation':
            dec_out = torch.stack([self.imputation(x_enc[:, :, feature], x_mark_enc, x_dec, x_mark_dec, mask) for feature in range(x_enc.shape[-1])],dim=-1)
            return dec_out  # [B, L, D]
        if self.task_name == 'anomaly_detection':
